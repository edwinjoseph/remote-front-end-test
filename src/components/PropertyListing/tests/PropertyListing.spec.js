...
describe('PropertyListing', () => {
    it('should render without crashing', () => {
        const wrapper = shallow(<PropertyListing loading={false} error={null} items={[DUMMY_PROPERTY]} />);
        expect(wrapper.find('.PropertyListing')).toHaveLength(1);
    });

    it('should render five property cards', () => {
        const wrapper = shallow(<PropertyListing loading={false} error={null} items={Array(5).fill(DUMMY_PROPERTY)} />);
